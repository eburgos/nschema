...
${routeArguments.map(argument => {
            return `    input${helpers_1.renderPropertyAccessor(argument.name)} = ${common_1.realTypeMap(argument, `unescape(event.pathParameters${helpers_1.renderPropertyAccessor(argument.name)})`)};
`;
        })}${queryArguments.map(argument => {
            return `    input${helpers_1.renderPropertyAccessor(argument.name)} = ${common_1.realTypeMap(argument, `event.queryStringParameters${helpers_1.renderPropertyAccessor(argument.name)}`)};`;
        })}
${headerArguments
            .map(argument => {
            return `    input${helpers_1.renderPropertyAccessor(argument.name)} = ${common_1.realTypeMap(argument, `event.headers${helpers_1.renderPropertyAccessor(argument.headerName || `X-${argument.name}`)}`)};`;
        })
            .join("\n")}
${bodyArguments.length === 1
    ...
    const $result: ${__1.messageType(nschema, context, true, outMessage)} = await $service.${operationName}(${(inMessage.data || []).length === 1
            ? "$req, "
            : (inMessage.data || [])
                .map(argument => {
