...
function findTypeMap(t, skipError) {
    switch (t) {
        case "int":
            return "number";
        case "float":
...
function typeName(nschemaType, nschema, namespace, name, context, addFlowComment) {
    let result;
    if (typeof nschemaType === "string") {
        ...
        if (isUnions(nschemaType)) {
            ...
            if (typeof findTypeMap(nschemaType.name, true) ===
                "string") {
...
function messageType($nschema, $nschemaMessage, $nschemaMessageDirection, $context) {
    ...
    const result = $_dataItems.length === 0
        ...
        : $_dataItems.length === 1
            ? [
                typeName($_dataItems[0].type, $nschema, $nschemaMessage.namespace, $nschemaMessage.name, $context)
            ]
            : $_dataItems.map((item, $i) => {
                return `${item.name || `item${$i}`}: ${typeName(item.type, $nschema, $nschemaMessage.namespace, $nschemaMessage.name, $context)}`;
            });
