    ...
    return `{
      ...
      url: \`\${this.${endpointPropertyName}}${routePrefix}${paramsInRoute.length
        ...
        ? `?\${[${`${paramsInQuery
            ...
            .join(`, `)}].filter((item: string | { name: string; value: string | number | undefined }) => (typeof item === "string")? item : (typeof(item.value) !== "undefined")).map(item => typeof item === "string"? item : \`\${item.name}=\${encodeURIComponent(\`\${item.value}\`)}\`).join("&")}`}`
        : ``}\`
    }`;
}
