...
function renderClass(data, nschema, context) {
    return `interface ${data.name}${data.implements && data.implements.length
        ? ` extends ${data.implements
            .map(impl => helpers_1.typeName(impl, nschema, data.namespace || "", data.name, context))
            .join(", ")}`
        : ""} {
${Object.keys(data.properties || {})
   ...
   * ${(property.description || "").replace(/\n/g, "\n     * ")}
   */
  ${property.typescriptName || prop}${isOptional ? "?" : ""}: ${helpers_1.typeName(nschemaType, nschema, data.namespace, data.name, context)};
`;
    })
        .join("\n")}}
`;
