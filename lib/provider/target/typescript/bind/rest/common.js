...
function isPrimitiveOptional(p) {
    const t = getType(p);
    return (t.namespace === "" &&
        utils_1.isPrimitiveTypeString(t.name) &&
        (t.modifier === "option" ||
            (util_1.isArray(t.modifier) &&
                t.modifier.length === 1 &&
                t.modifier[0] === "option")));
}
...
function includeInQuery(p) {
    ...
    return (p.paramType === "query" &&
        (!t.modifier ||
            (util_1.isArray(t.modifier) && !t.modifier.length) ||
...
function getOperationDetails(operation, name) {
    ...
    const paramsInRoute = allParams
        ...
        .map(p => {
        return Object.assign(Object.assign({}, p), { realType: getType(p), type: {
                name: "string",
                namespace: ""
            } });
    });
