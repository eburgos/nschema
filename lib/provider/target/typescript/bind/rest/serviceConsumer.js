...
function renderConstructorForClass(_nschema, context, _config, operations) {
    return Object.keys(operations)
        ...
        return `    expressApp.${common_1.getHttpVerb(operations[operationName].method || "get").toLowerCase()}("/${_config.routePrefix}${(route || operationName).replace(/\{([^}]+?)\}/g, (_match, firstGroup) => {
            return `:${firstGroup}`;
...
${routeArguments
            .map((argument) => {
            return `      const input${argument.name} = ${common_1.realTypeMap(argument, `expressRequest.params${helpers_1.renderPropertyAccessor(argument.name)}`)};
`;
        })
            .join("")}${queryArguments.map((argument) => {
            return `        const input${argument.name} = ${common_1.realTypeMap(argument, `expressRequest.query${helpers_1.renderPropertyAccessor(argument.name)}`)};
        `;
        }).join(`
        `)}${headerArguments.map((argument) => {
            return `const input${argument.name} = ${common_1.realTypeMap(argument, `expressRequest.header('${argument.headerName || `X-${argument.name}`}') || ""`)};`;
        }).join(`
