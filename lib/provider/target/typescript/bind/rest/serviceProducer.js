...
function buildRequest(method, route, routePrefix, endpointPropertyName, paramsInQuery, paramsInBody, paramsInHeader, paramsInRoute) {
    return `{
...
`;
    ...
    [__1.RestClientStrategy.Default]: (endpointPropertyName) => {
        return `  /**
   * Base url for this http service
   */
...
const requestOptionsPart = {
    ...
    [__1.RestClientStrategy.Default]: (method, route, routePrefix, endpointPropertyName, paramsInQuery, paramsInBody, paramsInHeader, paramsInRoute) => {
        return `: ${requestArgsType(method)} = ${buildRequest(method, route, routePrefix, endpointPropertyName, paramsInQuery, paramsInBody, paramsInHeader, paramsInRoute)};`;
    }
};
const bodyPart = {
...
let $body = JSON.stringify(${paramsInBody.length > 1 ? `[` : ``}${paramsInBody
    ...
    [__1.RestClientStrategy.Default]: (method, _route, _paramsInQuery, _paramsInBody, _paramsInHeader, _paramsInRoute, nschema, context, outMessage, _op, optionsVarName, _endpointPropertyName) => {
        ...
        return `const ${responseVarName} = await request.${method.toLowerCase()}${["delete"].indexOf(method.toLowerCase()) < 0
...
function renderOperations(nschema, context, config, deferredType, restClientStrategy, endpointPropertyName) {
...
`;
  ...
  public async ${op}(${(inMessage.data || [])
    ...
    const ${optionsVarName}${requestOptionsPart[restClientStrategy](method, route, routePrefix, endpointPropertyName, queryArguments, bodyArguments, headerArguments, routeArguments)}
    ${bodyPart[restClientStrategy](method, route, queryArguments, bodyArguments, headerArguments, routeArguments, nschema, context, outMessage, op, optionsVarName, endpointPropertyName)}
  }
`;
    })
  ...
  ${Object.keys(config.producerContexts || {})
            ...
            const operationArguments = pContext.arguments.map(arg => {
                const { isValidType, lastType } = Object.keys(contextOperations).reduce((acc, cop) => {
                    ...
                    const argumentTypeName = helpers_1.typeName(operationArgument.realType || operationArgument.type, nschema, config.namespace, config.name, context, true);
                    if (acc.isValidType &&
...
${renderOperations(nschema, context, config, deferredType, restClientStrategy, endpointPropertyName)}${errorHandlerPart[restClientStrategy]()}}
`;
}
