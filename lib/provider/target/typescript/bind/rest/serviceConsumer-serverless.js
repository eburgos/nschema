...
${bodyArguments.length === 1
    ...
    const $req = input as ${typescript_1.messageType(nschema, context, true, inMessage)};

    const $result: ${typescript_1.messageType(nschema, context, true, outMessage)} = await $service.${op}(${inMessage.data.length === 1
            ? "$req, "
            : inMessage.data
                .map(p => {
                return `$req.${p.name}, `;
...
//# sourceMappingURL=serviceConsumer-serverless.js.map