...
function requestArgsType(method, encoding) {
    return `{
      data: ${encoding === "json" ? `string` : `{ [name:string]: any } | undefined`};
      handleAs: string;
...
const bodyPart = {
    [__1.RestClientStrategy.Angular2]: (method, route, paramsInQuery, paramsInBody, paramsInHeader, paramsInRoute, _nschema, _context, _outMessage, operationName, optionsVarName, endpointPropertyName) => {
    ...
    ${optionsVarName}.headers =  new Headers({
      ...
      ${paramsInHeader
            .map((argument) => {
            return `"${argument.headerName || `X-${argument.name}`}": ${argument.name}`;
        })
            .join(",\n                ")}
    });
...
let $body = JSON.stringify(${paramsInBody.length > 1 ? `[` : ``}${paramsInBody
    ...
    [__1.RestClientStrategy.Default]: (method, _route, _paramsInQuery, _paramsInBody, _paramsInHeader, _paramsInRoute, nschema, context, outMessage, operationName, optionsVarName, _endpointPropertyName, errorHandlerPropertyName) => {
...
function renderOperations(nschema, context, config, deferredType, restClientStrategy, endpointPropertyName, errorHandlerPropertyName) {
    ...
    return `${Object.keys(config.operations)
        ...
        if (method === "get" && bodyArguments.length) {
            throw new Error(`Service "${config.name}" : operation "${operationName}" has method GET and body parameters "${bodyArguments
                .map((argument) => argument.name)
...
`;
  ...
  public async ${operationName}(${(inMessage.data || [])
            .map((par) => {
            return `${par.name}: ${helpers_1.typeName(par.type, nschema, config.namespace, config.name, context, true, true, true)}`;
        })
    ...
    ${bodyPart[restClientStrategy](method, route, queryArguments, bodyArguments, headerArguments, routeArguments, nschema, context, outMessage, operationName, optionsVarName, endpointPropertyName, errorHandlerPropertyName)}
  }
`;
    })
...
${Object.keys(config.producerContexts || {})
            ...
            const operationArguments = pContext.arguments.map((arg) => {
                const { isValidType, lastType } = Object.keys(contextOperations).reduce((acc, cop) => {
                    ...
                    const argumentTypeName = helpers_1.typeName(operationArgument.realType || operationArgument.type, nschema, config.namespace, config.name, context, true, true, true);
                    if (acc.isValidType &&
...
${Object.keys(contextOperations)
                ...
                return `      ${contextOperationName}: (${(operation.inMessage.data || [])
                    ...
                    .map((arg) => `${arg.name}: ${helpers_1.typeName(arg.realType || arg.type, nschema, config.namespace, config.name, context, true, true, true)}`)
...
${Object.keys(config.operations)
        ...
        return `  ${operationName}?(error: Error): ${deferredType}<${__1.messageType(nschema, context, false, outMessage)}>;`;
    })
        .join("\n")}
}
