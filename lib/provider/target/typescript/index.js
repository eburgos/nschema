...
function messageType(nschema, context, addFlowComment, message) {
    ...
    if (dataItems.length === 0) {
        ...
        return `${helpers_1.typeName(item.type, nschema, "", "", context, addFlowComment, false)}`;
    }
    else {
        return (`{ ${dataItems
            .map((item, $i) => {
            return `${item.name || `item${$i}`}: ${helpers_1.typeName(item.type, nschema, "", "", context, addFlowComment, false)}`;
        })
