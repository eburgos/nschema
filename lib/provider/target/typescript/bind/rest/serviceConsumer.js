...
function renderConstructorForClass(nschema, context, config, operations) {
    return Object.keys(operations)
        .map((operationName) => {
        const operation = operations[operationName];
        const { bodyArguments, headerArguments, inMessage, outMessage, route, routeArguments, queryArguments } = common_1.getOperationDetails(operation, operationName);
        ...
        return `    expressRouter.${common_1.getHttpVerb(operations[operationName].method || "get").toLowerCase()}<ParamsDictionary, ${__1.messageType(nschema, context, outMessage)} | string, /* ${bodyArguments.length} */ ${bodyArguments.length === 1
            ? `${helpers_1.typeName(bodyArguments[0].realType || bodyArguments[0].type, nschema, config.namespace, config.name, context, true, true)}`
            : `${bodyArguments.length
                ? `[${bodyArguments.map((argument) => {
                    return `${helpers_1.typeName(argument.realType || argument.type, nschema, config.namespace, config.name, context, true, true)}`;
                }).join(`,
            `)}]`
                : `undefined`}`}, ${queryArguments.length === 0
            ? "Record<string, never>"
            : `{${queryArguments
                .map((queryArg) => `${queryArg.name}: string`)
                .join(`;`)}}`}>("/${config.routePrefix}${(route || operationName).replace(/\{([^}]+?)\}/g, (_match, firstGroup) => {
            return `:${firstGroup}`;
...
`;
            ...
            .join(", ")}${(inMessage.data || []).length ? ", " : ""}{ request: expressRequest, response: expressResponse }));
            }
            catch (exception) {
