...
function renderClass(data, nschema, context) {
    return `interface ${data.name}${data.implements && data.implements.length
        ? ` extends ${data.implements
            .map(impl => helpers_1.typeName(impl, nschema, data.namespace || "", data.name, context, false, false))
            .join(", ")}`
        : ""} {
${Object.keys(data.properties || {})
   ...
   * ${(property.description || "").replace(/\n/g, "\n     * ")}
   */
  ${property.typescriptName || prop}${canOmitProperty ? "?" : ""}: ${helpers_1.typeName(canOmitProperty ? utils_1.removeOptional(nschemaType) : nschemaType, nschema, data.namespace, data.name, context, false, false)};
`;
    })
        .join("\n")}}
`;
