...
function computeImportMatrix(localNamespace, namespaceMapping, context) {
    ...
    const sortedImports = Object.keys(rootContext.imports)
        ...
        return {
            ...
            modulePath: contextImport.startsWith("{") && contextImport.endsWith("}")
                ? contextImport.slice(1, contextImport.length - 1)
                : namespaceMapping[contextImport] ||
                    (utils_1.isRelativePath(contextImport)
                        ? contextImport
                        : `./${contextImport}`),
            name: contextImport
        };
    ...
    const lines = sortedImports.map((sortedImport) => {
        ...
        const importNames = sorted.map((importName) => typeof sortedImport.imports[importName] === "string"
...
function messageType(nschema, nschemaMessage, nschemaMessageDirection, context) {
    ...
    const result = dataItems.length === 0
        ...
        : dataItems.length === 1
            ? [
                typeName(dataItems[0].type, nschema, nschemaMessage.namespace, nschemaMessage.name, context, true, false, true)
            ]
            : dataItems.map((item, itemIndex) => {
                return `${item.name || `item${itemIndex}`}: ${typeName(item.type, nschema, nschemaMessage.namespace, nschemaMessage.name, context, true, false, true)}`;
            });
