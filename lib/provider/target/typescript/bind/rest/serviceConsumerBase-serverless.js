...
function renderOperationsInterface(nschema, context, operations, name, namespace) {
    return Object.keys(operations)
        ...
        const contextVariable = utils_1.findNonCollidingName("context", (inMessage.data || []).map(d => d.name));
        return `
  /**
...
${(inMessage.data || [])
  ...
  ${op}(${(inMessage.data || [])
            ...
            .join(", ")}${(inMessage.data || []).length ? `, ` : ``}${contextVariable}: { event: any, context: any }): Promise<${__1.messageType(nschema, context, true, outMessage)}>;`;
    })
        .join("\n");
}
...
${(inMessage.data || [])
  ...
  public abstract async ${op}(${(inMessage.data || [])
            ...
            .join(", ")}${(inMessage.data || []).length ? `, ` : ``}$ctx?: { event: any, context: any } /*: { event: any, context: any } */): Promise<${__1.messageType(nschema, context, true, outMessage)}>;
`;
    })
        .join("\n");
