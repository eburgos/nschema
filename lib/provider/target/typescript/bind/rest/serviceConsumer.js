...
${inMessage.data
  ...
  ${op}(${inMessage.data
            ...
            .join(", ")}${inMessage.data.length ? `, ` : ``}$ctx: { request: Request, response: Response }): Promise<${__1.messageType(nschema, context, true, outMessage)}>;`;
    })
        .join("\n");
}
...
${inMessage.data
  ...
  public abstract async ${op}(${inMessage.data
            ...
            .join(", ")}${inMessage.data.length ? `, ` : ``}$ctx?: { request: Request, response: Response } /*: { request: Request, response: Response } */): Promise<${__1.messageType(nschema, context, true, outMessage)}>;
`;
    })
        .join("\n");
...
function renderConstructorForClass(nschema, context, config, operations) {
    return Object.keys(operations)
        .map(op => {
        const operation = operations[op];
        const { bodyArguments, headerArguments, inMessage, outMessage, route, routeArguments, queryArguments } = common_1.getOperationDetails(operation);
