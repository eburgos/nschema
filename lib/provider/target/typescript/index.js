...
exports.formatCode = exports.messageType = exports.buildTypeScriptContext = exports.TypeScript = exports.RestClientStrategy = exports.enableImport = void 0;
...
function messageType(nschema, context, message) {
    ...
    if (dataItems.length === 0) {
        ...
        return `${helpers_1.typeName(item.type, nschema, "", "", context, false, true)}`;
    }
    else {
        return (`{ ${dataItems
            .map((item, mapIndex) => {
            return `${item.name || `item${mapIndex}`}: ${helpers_1.typeName(item.type, nschema, "", "", context, false, true)}`;
        })
