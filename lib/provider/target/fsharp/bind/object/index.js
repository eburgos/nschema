...
function enumTemplate(data) {
  ...
  type ${data.name} =
      ${Object.keys(data.properties || {})
        ...
        | [<EnumMember>][<JsonProperty("${prop}")>] ${property.fsharpName || utils_1.initialCaps(prop)} = ${$currentVal}`;
    })
        .join("\n")}`;
}
function classTemplate(data, nschema) {
  ...
  ${Object.keys(data.properties || {})
        ...
        return `${utils_1.indent(indentIndex, "  ")}  /// <summary>${data.description || ""}</summary>
...
${utils_1.indent(indentIndex, "  ")}  `
            ...
            : `[<DataMember>][<JsonProperty("${prop}")>]`} member val ${property.fsharpName || utils_1.initialCaps(prop)}: ${__1.default.typeName($nschemaType, nschema, data.namespace || "")} = ${__1.typeDefaultValue($nschemaType, property, __1.default, nschema, data.namespace || "")} with get, set`;
    })
        .join("\n")}
`;
}
