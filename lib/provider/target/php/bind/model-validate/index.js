...
${renderValidationLogic(nschema, currentNamespace, name, typeof nschemaType !== "string"
                    ? Object.assign({}, nschemaType, { modifier: restModifiers }) : nschemaType, objIdentifier, errorsIdentifier, restModifiers, path, identifier, index, nest + 1)}
    ${indent}}`;
            case "array":
            case "list":
...
${renderValidationLogic(nschema, currentNamespace, name, typeof nschemaType !== "string"
                    ? Object.assign({}, nschemaType, { modifier: restModifiers }) : nschemaType, objIdentifier, errorsIdentifier, restModifiers, path, "$_obj", "$_idx", nest + 1)}
        $_idx += 1;
      }
    ${indent}} else {
...
function baseGenerate(config, nschema, target, template, context) {
...
${Object.keys(data.properties || {})
   ...
   * ${(property.description || "").replace(/\n/g, "\n   * ")}
   *
   * ${prop}: ${helpers_1.typeName(nschemaType, nschema, data.namespace, data.name, context, false, false, true)}
   */
`;
    })
        .join("\n")}
